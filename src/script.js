(() => {
  ...
  const MAX_ON_SCREEN = 180;
  const BOTTOM_FADE_ZONE = 120;
  ...
  function spawnSnowflake(x, y) {
    ...
    const blur = rand(0, 0.6);
    const spinDir = Math.random() < 0.5 ? -1 : 1;
    const spinSpeed = rand(180, 720) * spinDir;

    const startX = x + rand(-8, 8);
    const startY = y + rand(-8, 8);
    const fallSpeed = rand(140, 520);
    const driftAmplitude = rand(20, 180);
    const driftFreq = rand(0.6, 1.8);
    const wind = rand(-60, 60);
    const lifetimeCap = rand(2.6, 5.2);
  ...
  function animate(now) {
    ...
    for (const item of active) {
      ...
      const drift = Math.sin(t * Math.PI * 2 * item.driftFreq) * item.driftAmplitude
                  + item.wind * t;
